...
void MySelector::Begin(TTree * /*tree*/)
{
  ...
  for( const auto & chn:channels ){
    ...
    h_mll[chn] = new TH1D("h_"+chn+"_mll", chn+"_mll", 150, 0, 6000); //150 bins for invariant mass histogram between 0 and 6000 GeV	
    //logarithmic binning histogram
    h_mllLog[chn]=new TH1D("h_"+chn+"_mllLog",chn+"_mllLog", nbins, xbins);	
  }
...
Bool_t MySelector::Process(Long64_t entry)
{
  ...
  if(!(*lep_isTightID)[1]){ return kTRUE; }
  ...
  // Cut on z_0 impact parameter in milimeters
  ...
  h_mllLog[channel]->Fill(dileptons.M(), wgt);
...
void MySelector::WriteToFile(TString fileid, TString data_type)
{
  ...
  TFile file("Escritorio/CodeExample/Histograms/"+data_type+"/"+filename, "RECREATE"); 
